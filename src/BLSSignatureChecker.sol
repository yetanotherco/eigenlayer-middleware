...
contract BLSSignatureChecker is IBLSSignatureChecker {
    ...
    bytes constant ALIGNED_QUORUM_NUMBER = hex"00";
    ...
    {
        require(ALIGNED_QUORUM_NUMBER.length != 0, "BLSSignatureChecker.checkSignatures: empty quorum input");
...

        {
            ...
            // ALIGNED_QUORUM_NUMBER contains only unique, valid quorum numbers
            uint256 signingQuorumBitmap = BitmapUtils.orderedBytesArrayToBitmap(ALIGNED_QUORUM_NUMBER, registryCoordinator.quorumCount());
         ...
         */
        {
            ...
            for (uint256 i = 0; i < ALIGNED_QUORUM_NUMBER.length; i++) {
                ...
                if (_staleStakesForbidden) {
                    require(
                        registryCoordinator.quorumUpdateBlockNumber(uint8(ALIGNED_QUORUM_NUMBER[i])) + withdrawalDelayBlocks > referenceBlockNumber,
                ...
                for (uint256 j = 0; j < params.nonSignerPubkeys.length; j++) {
                    ...
                    if (BitmapUtils.isSet(nonSigners.quorumBitmaps[j], uint8(ALIGNED_QUORUM_NUMBER[i]))) {
